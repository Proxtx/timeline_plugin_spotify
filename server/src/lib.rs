use {
    base64::Engine, rocket::{futures, get, routes, State}, serde::{Deserialize, Serialize}, server_api::{cache::Cache, db::{Database, Event}, external::{futures::StreamExt, tokio::sync::RwLock, toml, types::{api::{APIResult, CompressedEvent}, available_plugins::AvailablePlugins, external::{chrono::{self, Utc}, mongodb::{bson::doc, options::FindOneOptions}, reqwest, serde_json}, timing::{TimeRange, Timing}}, url::Url}, plugin::{PluginData, PluginTrait}}, std::sync::Arc
};
...
impl PluginTrait for Plugin {
    ...
    where
        Self: Sized,
    {
        AvailablePlugins::timeline_plugin_spotify
    }

    fn get_compressed_events(&self, query_range: &TimeRange) -> std::pin::Pin<
        Box<
